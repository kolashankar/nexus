...
const mockStocks = [
    { ticker: 'ROBOT', name: 'Robot Corp', price: 150.50, change_24h: 5.2 },
    { ticker: 'CYBER', name: 'Cyber Security Inc', price: 89.75, change_24h: -2.1 },
    { ticker: 'TRADE', name: 'Trade Alliance', price: 220.00, change_24h: 0.5 },
];
describe('MarketStocks Component', () => {
    test('renders stock list', () => {
        render(_jsx(MarketStocks, { stocks: mockStocks }));
    ...
    test('shows portfolio if user owns stocks', () => {
        const portfolio = [{ ticker: 'ROBOT', shares: 10, avg_price: 145.00 }];
        render(_jsx(MarketStocks, { stocks: mockStocks, portfolio: portfolio }));
