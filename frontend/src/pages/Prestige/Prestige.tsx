...
import type { PlayerPrestige, PrestigeEligibility } from '../../types/prestige';
import { toast } from '../../components/ui/sonner';
import { Crown, Sparkles } from 'lucide-react';

const Prestige: React.FC = () => {
  const [prestige, setPrestige] = useState<PlayerPrestige | null>(null);
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-64">Loading prestige...</div>;
  }
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(prestige.permanent_bonuses).map(([bonus, value]) => (
                    <div key={bonus} className="flex items-center justify-between p-3 bg-primary/10 rounded-lg">
                      <span className="capitalize">{bonus.replace('_', ' ')}</span>
                      <Badge variant="secondary">x{value.toFixed(2)}</Badge>
                    </div>
                  ))}
                </div>
              </CardContent>
