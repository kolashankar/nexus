...
class ProgressionService {
  ...
  async unlockSkillNode(trait: string, nodeId: number) {
  ...
  async getSkillTree(trait: string) {
    const response = await api.get(`/player/skill-trees/${trait}`);
    return response.data;
  }

  async activateSuperpower(powerId: string) {
  ...
  async purchaseLegacyPerk(perkId: string) {
