...
export const useBattlePass = () => {
    ...
    const claimRewards = async (tier) => {
        const token = localStorage.getItem('token');
        const response = await axios.post(`${API_URL}/api/seasonal/battle-pass/claim-rewards`, { tier }, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
    ...
    const purchasePremium = async () => {
        const token = localStorage.getItem('token');
        const response = await axios.post(`${API_URL}/api/seasonal/battle-pass/purchase-premium`, {}, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
