...
export interface CombatAnimation {
  type: 'attack' | 'defend' | 'power' | 'damage' | 'heal' | 'miss';
  element: HTMLElement;
  duration?: number;
}
...
export const playCombatAnimation = ({
  ...
  element,
  duration = 500,
}: CombatAnimation): Promise<void> => {
  return new Promise((resolve) => {
    const animations: Record<string, string> = {
      attack: 'animate-combat-hit',
      defend: 'animate-shake',
      power: 'animate-power-activate',
      damage: 'animate-shake',
      heal: 'animate-glow',
      miss: 'animate-fade-out',
    };
...
export const createCombatParticles = (
  ...
  for (let i = 0; i < count; i++) {
    ...
    particle.style.setProperty('--x', Math.cos(angle).toString());
    particle.style.setProperty('--y', Math.sin(angle).toString());
