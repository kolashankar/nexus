...
const server = setupServer(rest.post('/api/combat/challenge', (req, res, ctx) => {
    return res(ctx.json({
        success: true,
        battle_id: 'battle123',
...
describe('Combat Flow Integration Tests', () => {
    ...
    test('use special ability', async () => {
        server.use(rest.post('/api/combat/action', (req, res, ctx) => {
            return res(ctx.json({
                ...mockCombatState,
        ...
        await waitFor(() => {
            expect(screen.getByText(/AP: 2/i)).toBeInTheDocument();
            expect(screen.getByText(/Heavy Attack/i)).toBeInTheDocument();
        });
    });
    test('combat victory flow', async () => {
        server.use(rest.get('/api/combat/state', (req, res, ctx) => {
            return res(ctx.json({
                ...mockCombatState,
                status: 'ended',
                winner: 'player123',
    ...
    test('flee from combat', async () => {
        server.use(rest.post('/api/combat/flee', (req, res, ctx) => {
            return res(ctx.json({
                success: true,
