...
describe('Marketplace Flow Integration Tests', () => {
    ...
    describe('Robot Marketplace', () => {
        ...
        it('should purchase a robot from marketplace', async () => {
            ...
            const listingResponse = await axios.get(`${API_URL}/api/robots/marketplace`, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
            if (listingResponse.data.length > 0) {
                const robotToBuy = listingResponse.data[0];
                const response = await axios.post(`${API_URL}/api/robots/purchase`, {
        ...
        it('should sell a robot', async () => {
            if (testRobotId) {
                const response = await axios.post(`${API_URL}/api/robots/sell`, {
    ...
    describe('Stock Market', () => {
        ...
        it('should get stock details', async () => {
            ...
            if (stocksResponse.data.length > 0) {
                ...
                const response = await axios.get(`${API_URL}/api/market/stocks/${ticker}`, {
                    headers: { Authorization: `Bearer ${authToken}` },
                });
                expect(response.status).toBe(200);
        ...
        it('should buy stocks', async () => {
            ...
            if (stocksResponse.data.length > 0) {
                const ticker = stocksResponse.data[0].ticker;
                const response = await axios.post(`${API_URL}/api/market/stocks/buy`, {
                    ticker,
                    quantity: 10,
                }, {
                    headers: { Authorization: `Bearer ${authToken}` },
        ...
        it('should sell stocks', async () => {
            const portfolioResponse = await axios.get(`${API_URL}/api/market/portfolio`, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
            if (portfolioResponse.data.length > 0) {
                const holding = portfolioResponse.data[0];
                const response = await axios.post(`${API_URL}/api/market/stocks/sell`, {
    ...
    describe('Item Marketplace', () => {
        ...
        it('should buy an item', async () => {
            ...
            if (itemsResponse.data.length > 0) {
                ...
                const response = await axios.post(`${API_URL}/api/market/items/buy`, {
