...
async def hack_player(
    ...
    hacker_skill = current_user.traits.hacking + current_user.traits.technical_knowledge
    target_defense = target.traits.technical_knowledge + target.meta_traits.tactical_mastery
    
    success_chance = min(80, max(20, (hacker_skill / (hacker_skill + target_defense)) * 100))
    ...
    if success:
        ...
        if stolen_amount > 0:
            ...
            # Calculate karma
            result = calculate_basic_karma("hack", current_user, target, stolen_amount)
...
async def steal_from_player(
    ...
    if target.id == current_user.id:
        raise HTTPException(status_code=400, detail="Cannot steal from yourself")
    
    # Calculate success chance (stealth-based)
    thief_skill = current_user.traits.stealth + current_user.traits.dexterity
    target_awareness = target.traits.perception + target.meta_traits.survival_instinct
    
    success_chance = min(75, max(15, (thief_skill / (thief_skill + target_awareness)) * 100))
    success = random.random() * 100 < success_chance
    ...
    if success:
        ...
        if stolen_amount > 0:
            ...
            # Calculate karma
            result = calculate_basic_karma("steal", current_user, target, stolen_amount)
...
async def donate_to_player(
    ...
    # Calculate karma
    result = calculate_basic_karma("donate", current_user, target, action.amount)
...
async def trade_with_player(
    ...
    if current_user.currencies.credits < action.offer_amount:
        raise HTTPException(status_code=400, detail="Insufficient credits for offer")
    
    if target.currencies.credits < action.request_amount:
        raise HTTPException(status_code=400, detail="Target has insufficient credits")
    ...
    # Calculate karma
    result = calculate_basic_karma("trade", current_user, target, action.offer_amount)
