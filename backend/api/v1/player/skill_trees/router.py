...
async def get_skill_tree(
    ...
    if not skill_trees_data:
        raise HTTPException(status_code=404, detail="Skill trees not initialized")
    
    skill_trees = PlayerSkillTrees(**skill_trees_data)
    
    if trait_name not in skill_trees.skill_trees:
        raise HTTPException(status_code=404, detail=f"Skill tree not found: {trait_name}")
...
async def unlock_node(
    ...
    if not skill_trees_data:
        skill_trees = SkillTreeService.initialize_skill_trees(str(current_player["_id"]))
    else:
        skill_trees = PlayerSkillTrees(**skill_trees_data)
...
async def calculate_synergies(current_player: Dict = Depends(get_current_player)):
    ...
    skill_trees = PlayerSkillTrees(**skill_trees_data)
    synergies = SkillTreeService.calculate_synergy_bonuses(skill_trees, player_traits)
