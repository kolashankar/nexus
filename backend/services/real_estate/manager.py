...
class RealEstateService:
    ...
    async def sell_property(
        ...
        # Remove property
        player_properties = [p for p in player_properties if p.get("property_id") != property_id]
    ...
    async def upgrade_property(
        ...
        player_properties = player.get("properties", [])
        property_index = next(
            (i for i, p in enumerate(player_properties) if p.get("property_id") == property_id),
            None
        )
        ...
        property_to_upgrade["passive_income"] = property_to_upgrade.get("passive_income", 0) + upgrade["income_boost"]
    ...
    async def calculate_property_income(self, player_id: str) -> Dict:
        ...
        player_properties = player.get("properties", [])
        total_income = sum(p.get("passive_income", 0) for p in player_properties)
