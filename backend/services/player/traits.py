...
class TraitsService:
    ...
    def get_top_traits(self, player: Player, limit: int = 10) -> Dict:
        ...
        all_traits = {**player.traits.model_dump(), **player.meta_traits.model_dump()}
        sorted_traits = sorted(all_traits.items(), key=lambda x: x[1], reverse=True)
        
        return {
            "top_traits": [
                {"name": name, "value": value, "category": self._get_trait_category(name)}
    ...
    def get_trait_details(self, player: Player, trait_name: str) -> Dict:
        ...
        description = self.TRAIT_DESCRIPTIONS.get(trait_name, "No description available")
