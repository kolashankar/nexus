...
class InvestmentService:
    ...
    async def get_portfolio(self, player_id: str) -> Dict:
        ...
        total_invested = sum(inv.get("amount_invested", 0) for inv in investments)
        ...
        for inv in investments:
            ...
            days_held = (datetime.utcnow() - inv.get("investment_date", datetime.utcnow())).days
            ...
            current_val = int(inv.get("amount_invested", 0) * (1 + total_return))
            
            updated_inv = inv.copy()
            updated_inv["current_value"] = current_val
            updated_inv["profit_loss"] = current_val - inv.get("amount_invested", 0)
            updated_inv["profit_loss_percentage"] = (
                (current_val - inv.get("amount_invested", 0)) / inv.get("amount_invested", 1) * 100
            )
    ...
    async def withdraw_investment(
        ...
        investment_index = next(
            (i for i, inv in enumerate(player_investments) if inv.get("id") == investment_id),
            None
        )
        ...
        current_value = investment.get("current_value", investment.get("amount_invested", 0))
    ...
    async def calculate_performance(self, player_id: str) -> Dict:
        ...
        roi = (total_return / total_invested * 100) if total_invested > 0 else 0.0
        
        # Best and worst performing investments
        investments = portfolio["investments"]
        if investments:
            best_investment = max(investments, key=lambda x: x.get("profit_loss_percentage", 0))
            worst_investment = min(investments, key=lambda x: x.get("profit_loss_percentage", 0))
        else:
