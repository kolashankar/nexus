...
class WorldStateService:
    ...
    async def update_global_karma(
        ...
        if karma_change > 0:
            positive_actions = state["global_karma"].get("positive_actions_today", 0) + 1
            update = {"global_karma.positive_actions_today": positive_actions}
        else:
            negative_actions = state["global_karma"].get("negative_actions_today", 0) + 1
            update = {"global_karma.negative_actions_today": negative_actions}
        
        update["global_karma.collective_karma"] = new_karma
        update["global_karma.total_actions_today"] = state["global_karma"].get("total_actions_today", 0) + 1
