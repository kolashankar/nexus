...
class AICompanion(BaseAIService):
    ...
    async def talk(
        ...
        personality = self.personality_engine.get_personality(player.get("karma_points", 0))
        ...
        logger.info(f"Companion {companion_name} responded to {player.get('username')}")
    ...
    async def give_advice(
        ...
        logger.info(f"Companion {companion_name} gave advice to {player.get('username')}")
