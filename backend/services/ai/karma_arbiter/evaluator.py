...
class ActionEvaluator:
    ...
    async def evaluate(self, request: EvaluationRequest, use_cache: bool = True) -> EvaluationResponse:
        ...
        if use_cache:
            ...
            if cached_result:
                logger.info(f"Using cached evaluation for {request.context.action_type}")
                self.cost_tracker.track_call("karma_arbiter", MODEL_CONFIG["default_model"], 0, 0, cached=True)
    ...
    def _prepare_prompt(self, request: EvaluationRequest) -> str:
        ...
        return ACTION_EVALUATION_TEMPLATE.format(
            ...
            actor_recent_actions=", ".join(actor.recent_actions[-5:]) if actor.recent_actions else "None",
            actor_top_traits=self._format_traits(actor.traits, 60, above=True),
            actor_low_traits=self._format_traits(actor.traits, 40, above=False),
            ...
            target_top_traits=self._format_traits(target.traits, 60, above=True) if target else "N/A",
    ...
    def _fallback_evaluation(self, request: EvaluationRequest) -> EvaluationResponse:
        ...
        if action_type == "help":
            ...
            karma_change = -15
            trait_changes = {"hacking": 3, "deceit": 1}
