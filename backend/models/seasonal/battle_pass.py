...
class BattlePassTier(BaseModel):
    ...
    xp_required: int = Field(..., description="Cumulative XP required to reach this tier")
...
class BattlePass(BaseModel):
    ...
    premium_tiers: int = Field(default=100, description="Number of premium tiers")
    
    # Tiers data
    tiers: List[BattlePassTier] = Field(default_factory=list)
    
    # Pricing
    premium_price: int = Field(default=1000, description="Cost in credits to unlock premium")
...
class BattlePassReward(BaseModel):
    ...
    reward_id: str = Field(..., description="ID of the specific reward (cosmetic ID, etc.)")
    amount: int = Field(default=1, description="Quantity of the reward")
    name: str
    description: str = ""
    icon_url: Optional[str] = None
    rarity: Optional[str] = Field(default="common", description="common, rare, epic, legendary")
